...
| [`jco-wasi-http-testkit/`](./jco-wasi-http-testkit) | Node.js + jco + preview2-shim `HTTPServer` | Rust component (`wasi:http/proxy` world) | **HTTP リクエストハンドラを wasm 側に置く** Go httptest 風テストパターン。同じ component が production (wasmtime serve / Spin) でも本サンプル (loopback) でも走る |
